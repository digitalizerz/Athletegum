<x-superadmin-dashboard-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl font-semibold">Revenue by Deal</h2>
            <div class="flex gap-2 items-center flex-wrap">
                <x-date-filter :currentFilter="$filter ?? 'all'" routeName="admin.revenue.deals" />
                <a href="{{ route('admin.revenue.deals', array_merge(request()->except('page'), ['export' => 'csv', 'filter' => $filter ?? 'all'])) }}" 
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="card bg-base-100 shadow-sm border border-base-300">
                <div class="card-body">
                    <p class="text-sm font-medium text-base-content/60">Gross Volume</p>
                    <p class="text-2xl font-bold mt-1">${{ number_format($totals['gross_volume'], 2) }}</p>
                </div>
            </div>
            <div class="card bg-base-100 shadow-sm border border-base-300">
                <div class="card-body">
            ...
            <div class="card bg-base-100 shadow-sm border border-base-300">
                <div class="card-body">
                    <p class="text-sm font-medium text-base-content/60">Net Payouts</p>
                    <p class="text-2xl font-bold mt-1">${{ number_format($totals['net_payouts'], 2) }}</p>
                </div>
            </div>
        </div>
                    ...
                    <table class="table table-zebra">
                        ...
                        <tbody>
                            @forelse($deals as $deal)
                                ...
                                <tr>
                                    <td>
                                        <a href="{{ route('admin.deals.show', $deal) }}" class="link link-primary">
                                            #{{ $deal->id }}
                                        </a>
                                    </td>
                                    <td>{{ $dealTypeName }}</td>
                                    ...
                                    <td class="font-semibold">${{ number_format($deal->payout_amount ?? 0, 2) }}</td>
                                    <td>
