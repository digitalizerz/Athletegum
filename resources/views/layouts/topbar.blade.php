<header class="bg-base-100 border-b border-base-300 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8">
    <div class="flex items-center space-x-4">
        <!-- Mobile menu button -->
        <button @click="toggleSidebar()" class="lg:hidden p-2 rounded-md text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500" style="min-width: 44px; min-height: 44px; display: flex; align-items: center; justify-content: center;">
            <svg class="h-6 w-6 text-gray-900 dark:text-gray-100" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="stroke-width: 2;">
    ...
    <div class="flex items-center space-x-2 sm:space-x-3">
        ...
        <button 
            ...
            <!-- Moon icon (dark mode) - shown when in light mode (click to switch to dark) -->
            <svg x-show="!darkMode" class="h-5 w-5 text-gray-900 dark:text-gray-100" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="stroke-width: 2;">
        ...
        <div class="dropdown dropdown-end">
            <label tabindex="0" class="btn btn-ghost relative hover:bg-base-200 focus:bg-base-200" style="width: 44px; height: 44px; padding: 0; display: flex; align-items: center; justify-content: center; border-radius: 50%;">
                <svg class="flex-shrink-0 text-gray-900 dark:text-gray-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="width: 22px; height: 22px;">
            ...
            <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-80 p-2 shadow-lg border border-base-300 max-h-96 overflow-y-auto mt-2">
                ...
                @php
                    try {
                        $recentNotifications = \App\Models\Notification::where('user_type', 'user')
                            ->where('user_id', Auth::id())
                ...
                @if($recentNotifications->count() > 0)
                    <li class="border-t border-base-300 mt-2 pt-2">
                        <a href="{{ route('notifications.index') }}" class="text-center text-sm text-primary">View All Notifications</a>
                    </li>
                @endif
            </ul>
        </div>
        ...
        <div class="dropdown dropdown-end">
            <div tabindex="0" role="button" class="btn btn-ghost btn-sm" style="min-height: 44px;">
                <div class="flex items-center space-x-2 sm:space-x-3">
                    ...
                    <svg class="h-4 w-4 text-gray-900 dark:text-gray-100 hidden sm:block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
