<aside :class="sidebarOpen ? 'translate-x-0 lg:translate-x-0' : '-translate-x-full lg:-translate-x-full'"
       class="fixed inset-y-0 left-0 z-30 w-64 bg-base-100 border-r border-base-300 transition-transform duration-300 ease-in-out transform lg:fixed">
    <div class="flex flex-col h-full">
        <!-- Logo -->
        <div class="flex items-center justify-between h-16 px-6 border-b border-base-300">
            <a href="{{ route('dashboard') }}">
                <x-athletegum-logo size="md" text-color="default" />
            </a>
            <button @click="toggleSidebar()" class="lg:hidden btn btn-ghost btn-sm btn-square">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
        </div>

        <!-- Navigation -->
        <ul class="menu menu-vertical flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            @php
                use App\Support\PlanFeatures;
                $user = Auth::user();
                $plan = $user->subscription_plan ?? 'free';
                $status = $user->subscription_status ?? null;
                $planLabel = $plan === 'free' || ($plan !== 'free' && $status !== 'active') ? 'Free' : ucfirst($plan);
            @endphp
            
            <li>
                <a href="{{ route('dashboard') }}" 
                   class="{{ request()->routeIs('dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                    </svg>
                    Dashboard
                    <span class="ml-2 text-xs px-2 py-0.5 rounded {{ $planLabel === 'Free' ? 'bg-gray-200 text-gray-700' : ($planLabel === 'Pro' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700') }}">
                        {{ $planLabel }}
                    </span>
                </a>
            </li>

            <li>
                <a href="{{ route('deals.index') }}" 
                   class="{{ request()->routeIs('deals.index') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    Deals
                </a>
            </li>

            <li>
                <a href="{{ route('business.athletes.index') }}" 
                   class="{{ request()->routeIs('business.athletes.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    Athletes
                </a>
            </li>

            @if(PlanFeatures::canUseFeature($user, 'revenue_dashboard'))
                <li>
                    <a href="{{ route('business.revenue.index') }}" 
                       class="{{ request()->routeIs('business.revenue.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                        </svg>
                        Revenue
                    </a>
                </li>
            @endif
            
            @if($planLabel === 'Free')
                <li>
                    <a href="{{ route('business.billing.index') }}" 
                       class="{{ request()->routeIs('business.billing.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                        Upgrade
                    </a>
                </li>
            @endif

            <li>
                <a href="{{ route('business.billing.index') }}" 
                   class="{{ request()->routeIs('business.billing.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                    </svg>
                    Billing
                </a>
            </li>

            <li>
                <a href="{{ route('messages.index') }}" 
                   class="{{ request()->routeIs('messages.*') || request()->routeIs('deals.messages') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    Messages
                    @php
                        try {
                            $userId = Auth::id();
                            // Check if the column exists by trying to query it
                            $columns = \Schema::hasColumns('messages', ['read_by_user_ids']);
                            
                            if ($columns) {
                                // New read tracking logic
                                $unreadMessageCount = \App\Models\Message::whereHas('deal', function($query) use ($userId) {
                                    $query->where('user_id', $userId)
                                          ->where('status', '!=', 'pending')
                                          ->where('status', '!=', 'completed')
                                          ->where('status', '!=', 'cancelled')
                                          ->whereNull('released_at')
                                          ->whereNotNull('athlete_id');
                                })
                                ->where('sender_type', 'athlete')
                                ->get()
                                ->filter(function($message) use ($userId) {
                                    $readBy = $message->read_by_user_ids ?? [];
                                    return !in_array($userId, $readBy);
                                })
                                ->count();
                            } else {
                                // Fallback: count all messages (no read tracking yet)
                                $unreadMessageCount = \App\Models\Message::whereHas('deal', function($query) use ($userId) {
                                    $query->where('user_id', $userId)
                                          ->where('status', '!=', 'pending')
                                          ->where('status', '!=', 'completed')
                                          ->where('status', '!=', 'cancelled')
                                          ->whereNull('released_at')
                                          ->whereNotNull('athlete_id');
                                })
                                ->where('sender_type', 'athlete')
                                ->count();
                            }
                        } catch (\Exception $e) {
                            $unreadMessageCount = 0;
                        }
                    @endphp
                    @if($unreadMessageCount > 0)
                        <span class="badge badge-primary badge-sm">{{ $unreadMessageCount }}</span>
                    @endif
                </a>
            </li>

            <li>
                <a href="{{ route('deals.create') }}" 
                   class="{{ request()->routeIs('deals.create') || request()->routeIs('deals.create.*') || request()->routeIs('deals.review') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Create Deal
                </a>
            </li>

            <li>
                <a href="{{ route('wallet.index') }}" 
                   class="{{ request()->routeIs('wallet.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    Wallet
                </a>
            </li>

            <li>
                <a href="{{ route('payment-methods.index') }}" 
                   class="{{ request()->routeIs('payment-methods.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                    </svg>
                    Payment Methods
                </a>
            </li>

            @if(Auth::user()->is_superadmin)
            <li>
                <a href="{{ route('admin.settings') }}" 
                   class="{{ request()->routeIs('admin.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    Admin Settings
                </a>
            </li>
            @endif

            <li class="mt-auto pt-4 border-t border-base-300">
                <div class="menu-title">
                    <span class="text-xs font-semibold uppercase">Account</span>
                </div>
            </li>

            <li>
                <a href="{{ route('profile.edit') }}" 
                   class="{{ request()->routeIs('profile.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                    Profile
                </a>
            </li>
        </ul>
    </div>
</aside>
