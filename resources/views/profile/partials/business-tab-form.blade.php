@if(!$user->is_superadmin)
    <div x-data="{ editing: {{ ($errors->any() || session('status') === 'business-updated') ? 'true' : 'false' }} }">
        ...
        <div x-show="!editing" style="display: none;">
            <div class="space-y-4">
                <div>
                    <div class="text-sm text-base-content/60 mb-1">Business Name</div>
                    <div class="text-base font-medium">{{ $user->business_name ?: '—' }}</div>
                </div>
                
                <div>
                    <div class="text-sm text-base-content/60 mb-1">Business Information</div>
                    <div class="text-base whitespace-pre-wrap">{{ $user->business_information ?: '—' }}</div>
                </div>
                
                <div>
                    <div class="text-sm text-base-content/60 mb-1">Business Phone</div>
                    <div class="text-base">{{ $user->phone ?: '—' }}</div>
                </div>
                
                <div>
                    <div class="text-sm text-base-content/60 mb-1">Owner / Principal Name</div>
                    <div class="text-base">{{ $user->owner_principal ?: '—' }}</div>
                </div>
            </div>
        </div>
